...
foreach ($folder in $kyuFolders) {
    ...
    if ($fileCount -le $MaxFiles) {
        Write-Host "  ✓ Within limit, skipping" -ForegroundColor Green
        continue
    }
    
    Write-Host "  ⚠ Over limit ($fileCount > $MaxFiles), organizing..." -ForegroundColor Yellow
    ...
    for ($i = 0; $i -lt $subfoldersNeeded; $i++) {
        ...
        Write-Host "  ✓ Moved $movedCount files to $subfolderName" -ForegroundColor Green
    }
}
