...
function getSocks(name, socks) {
  while(socks.length){
    const curr = socks.shift();
    const diff = socks.find(color => color !== curr)
    const same = socks.find(color => color === curr)
    if(name === 'Punky' && diff) return [curr, diff]
    if(name === 'Henry' && same) return [curr, same]
  }
  return []
}