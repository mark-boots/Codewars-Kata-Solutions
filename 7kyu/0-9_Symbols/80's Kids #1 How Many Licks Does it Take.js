...
https://www.codewars.com/kata/566091b73e119a073100003a/solutions/javascript
...
function totalLicks(env) {
  env = Object.entries(env).sort((a,b) => b[1] - a[1]);
  const licks = 252 + env.reduce((t, [k, v]) => t + v, 0 );
  return `It took ${licks} licks to get to the tootsie roll center of a tootsie pop.` + (env[0] && env[0][1] > 0 ? ` The toughest challenge was ${env[0][0]}.` : '')
}