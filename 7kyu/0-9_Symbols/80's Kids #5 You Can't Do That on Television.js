...
const bucketOf = s => {
  ...
  const matched = Object.entries(triggers)
    .filter(([_, words]) => new RegExp(words.join("|"),"gi").test(s))
    .map(([key]) => key);

  return matched.length === Object.keys(triggers).length ? "sludge" : matched[0] || "air";
};
