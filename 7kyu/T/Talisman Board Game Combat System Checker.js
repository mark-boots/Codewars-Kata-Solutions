...
n the board game Talisman, when two players enter combat the outcome is decided by a combat score, equal to the players power plus any modifiers plus the roll of a standard 1-6 dice. The player with the highest combat score wins and the opposing player loses a life. In the case of a tie combat ends with neither player losing a life.
...
getRequired=([a,b],[c,d],p=a+b,e=c+d)=>
  p==e?"Random":
  p>=e+6?"Auto-win":
  p+6<=e?"Auto-lose":
  p>e?`(${e+7-p}..6)`:
...
getRequired=([a,b],[c,d],p=a+b,e=c+d)=>p==e?"Random":p>=e+6?"Auto-win":p+6<=e?"Auto-lose":p>e?`(${e+7-p}..6)`:p+5>e?`(1..${p+5-e})`:"Pray for a tie!"

getRequired=([a,b],[c,d])=>(a=>a>5?"Auto-win":a<-5?"Auto-lose":a==0?"Random":a==-5?'Pray for a tie!':a>0?`(${7-a}..6)`:`(1..${a+5})`)(a+b-c-d)