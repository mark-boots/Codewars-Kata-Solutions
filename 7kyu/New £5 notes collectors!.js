...
function getNewNotes(salary, bills) {
    const remaining = bills.reduce((acc, curr) => acc - curr, salary);
    const notes = Math.floor(remaining / 5);
    return notes < 0 ? 0 : notes;
}
// getNewNotes=(s,b)=>Math.max(b.reduce((a,v)=>a-v,s)/5|0,0)