...
function clonewars(kataPerDay) {
    let numberOfClones = Math.round(Math.pow(2, kataPerDay - 1)); 
    let katasSolvedByClones = 0;
    for (let i = 0; i < kataPerDay; i++) {
        katasSolvedByClones += Math.pow(2, i) * (kataPerDay - i);
    }
    return [numberOfClones, katasSolvedByClones];
}

// clonewars=k=>[Math.ceil(2**(k-1)),2**(k+1)-k-2]
