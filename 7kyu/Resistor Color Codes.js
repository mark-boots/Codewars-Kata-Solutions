...
const codes = { black: 0, brown: 1, red: 2, orange: 3, yellow: 4, green: 5, blue: 6, violet: 7, gray: 8, white: 9, gold: 5, silver: 10 }

function decodeResistorColors(bands) {
  const [first, second, power, tol=20] = bands.split(" ").map(v=>codes[v]);
  let val = (first * 10 + second) * Math.pow(10, power);
  val >= 1000000 ? val = val / 1000000 + 'M' : val >= 1000 ? val = val / 1000 + 'k' : "";
  return val + ' ohms, ' + tol + '%'
}
