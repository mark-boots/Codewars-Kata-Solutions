...
*/
const rings = ['abdegopqADOPQR','B'];
const results = ['Not even a medal!','Bronze!','Silver!','Gold!'];
const minmax = (val,min,max) => Math.max(Math.min(val,max),min)

const olympicRing = a => 
  (score = minmax(Math.floor([...a].reduce((t,v)=>t+(rings.findIndex(r=>r.includes(v))+1),0)/2),1,4),
  results[score-1])