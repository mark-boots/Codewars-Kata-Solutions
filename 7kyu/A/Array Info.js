...
function arrayInfo(arr) {
  ...
  let intCount = 0, floatCount = 0, strCount = 0, spaceCount = 0;

  for (let item of arr) {
    if (typeof item === 'number') Number.isInteger(item) ? intCount++ : floatCount++;
    if (typeof item === 'string') /^\s+$/.test(item) ? spaceCount++ : strCount++;
  }
