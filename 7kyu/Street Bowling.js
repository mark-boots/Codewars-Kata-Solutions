...
function ballTest(s, r) {
  const cracks = [...r].filter((t, i) => i < s && t == 'x').length;
  return s <= 0 ? false : s >=  r.length ? true : ballTest(s-1-cracks, r.slice(s))
}
//
ballTest=(s,r)=>s<=0?!1:s>=r.length?!0:ballTest(s-1-(r.slice(0,s).match(/x/g)||[]).length,r.slice(s))
//
ballTest=(s,r)=>!r||s>0&&ballTest(s-r.slice(0, s).split`x`.length,r.slice(s))
