...
*/
const solve = (s,CC=x=>x.charCodeAt()) => [...s].every((v,i)=>[0,2].includes(Math.abs(CC(v)-CC(s[s.length-i-1]))))

const solve = (s,CC=x=>x.charCodeAt()) => [...s].every((v,i)=>[-2,0,2].includes(CC(v)-CC(s[s.length-i-1])))