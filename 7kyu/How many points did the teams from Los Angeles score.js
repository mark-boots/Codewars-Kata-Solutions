...
function getLosAngelesPoints(results) {
    return results.reduce((sum, result)=>{
        return /^Los Angeles [A-Z][a-z]+$/.test(result[0]) ? sum + Number(result[1].split(":")[0]) : sum
    },0);  
}

//getLosAngelesPoints=r=>r.reduce((s,v)=>/^Los Angeles [A-Z][a-z]+$/.test(v[0])?s+Number(v[1].split(":")[0]):s,0)