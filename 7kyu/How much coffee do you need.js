...
function howMuchCoffee(events) {
    const coffees = events.reduce((s,v)=>{
        return /^(cw|cat|dog|movie)$/i.test(v) ? s+(v==v.toUpperCase()?2:1) : s
    },0);
    return coffees > 3 ? 'You need extra sleep' : coffees;
}

// howMuchCoffee=e=>(c=e.reduce((s,v)=>/^(cw|cat|dog|movie)$/i.test(v)?s+(v==v.toUpperCase()?2:1):s,0),c>3?'You need extra sleep':c)