...
https://www.codewars.com/kata/58881b859ab1e053240000cc/train/javascript
...
function electionsWinners(votes, k) {
  ...
  const winners = votes.filter(v => v + k > max).length;
  return k === 0
    ? votes.filter(v => v === max).length === 1 ? 1 : 0
    : winners;
}
// codegolf solution
electionsWinners=(v,k)=>(m=Math.max(...v),k?v.filter(x=>x+k>m).length:v.filter(x=>x==m).length==1?1:0)